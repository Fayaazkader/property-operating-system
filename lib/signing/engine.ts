...
export class SigningEngine {
  ...
  async completeSigning(
    ...
    const { data: certRecord } = await supabase.from('execution_certificates').insert({
      request_id: requestId, document_id: request.lease_id || requestId,
      pdf_url: certPdfUrl, signed_by_name: signerName, signed_by_email: signerEmail,
      fields_signed: certificate.fields_signed?.length || 0, hash: executionHash,
    }).select('id').single();

    // Update signing request
    await supabase.from('signature_requests').update({
      status: 'completed', completed_at: new Date().toISOString(),
      certificate_id: certRecord?.id, execution_package_hash: executionHash || null,
    }).eq('id', requestId);
    ...
    await supabase.from('execution_artifacts').insert([
      ...
      { entity_id: request.entity_id, artifact_type: 'audit_trail', artifact_data: { request_id: requestId, signed_by: { name: signerName, email: signerEmail }, signed_at: new Date().toISOString(), template_id: request.template_id, template_version: request.template_version } },
      { entity_id: request.entity_id, artifact_type: 'verification_data', artifact_data: { execution_hash: executionHash, algorithm: 'SHA-256', request_id: requestId, document_id: request.lease_id || requestId, signed_at: new Date().toISOString(), provider: 'native', template_version: request.template_version || 1 } },
    ]);

    // Store executed PDF
    await supabase.from('documents').insert({
      entity_id: request.entity_id, file_name: `executed-${request.document_name}`,
      file_url: request.document_url, mime_type: 'application/pdf', document_type: 'signed_lease',
      status: 'stored', related_entity_type: request.request_type === 'lease' ? 'lease' : 'document',
    ...
    if (request.request_type === 'lease' && request.lease_id) {
      ...
      if (newStatus === 'Pending Commencement') {
        await publish('lease.pending_commencement', { correlationId: crypto.randomUUID(), source: 'signing-engine', version: '1.0', payload: { leaseId: request.lease_id, entityId: request.entity_id, requestId } });
      }

      await publish('lease.execution.completed', { correlationId: crypto.randomUUID(), source: 'signing-engine', version: '1.0', payload: { requestId, leaseId: request.lease_id, entityId: request.entity_id, executionHash } });
      await publish('document.execution.package.created', { correlationId: crypto.randomUUID(), source: 'signing-engine', version: '1.0', payload: { requestId, entityId: request.entity_id, packageHash: executionHash } });
    }

    await publish('signing.request.completed', { correlationId: crypto.randomUUID(), source: 'signing-engine', version: '1.0', payload: { requestId, requestType: request.request_type } });
  }
