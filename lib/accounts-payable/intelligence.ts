...
export const apIntelligence = {
  async getWarnings(entityId: string): Promise<APWarning[]> {
    ...
    const { data: duplicates } = await supabase
      ...
      .group('supplier_id, invoice_number')
      .having('COUNT(*)', '>', '1');
    (duplicates || []).forEach((d: any) => {
      warnings.push({ type: 'duplicate', severity: 'high', message: `Duplicate invoice: ${d.invoice_number}`, supplierId: d.supplier_id });
    });
    ...
    for (const s of (inactiveSuppliers || [])) {
      const { count } = await supabase.from('supplier_invoices_new').select('*', { count: 'exact', head: true }).eq('supplier_id', s.id).eq('lifecycle_status', 'posted');
      if ((count || 0) > 0) {
    ...
    // Missing GL codes
    const { data: drafts } = await supabase.from('supplier_invoices_new').select('id, invoice_number').eq('entity_id', entityId).eq('lifecycle_status', 'captured');
    for (const d of (drafts || [])) {
      const { data: lines } = await supabase.from('supplier_invoice_lines').select('gl_code').eq('invoice_id', d.id);
      if ((lines || []).some((l: any) => !l.gl_code)) {
        warnings.push({ type: 'missing_gl', severity: 'medium', message: `Missing GL code: ${d.invoice_number}`, invoiceId: d.id });
      }
    }
  ...
  async getCodingSuggestions(supplierId: string): Promise<CodingSuggestion | null> {
    ...
    const { data: lines } = await supabase
      ...
      .select('gl_code, property_id, vat_code')
