...
export class FinancialGovernanceEngine {
  async validateEvent(
    ...
    if (!event.entity_id)
      return { valid: false, reason: "Entity ID is required" };
    if (!event.amount || event.amount <= 0)
      return { valid: false, reason: "Amount must be greater than zero" };
    if (!event.business_event)
      return { valid: false, reason: "Business event is required" };
  ...
  async runIntegrityChecks(
    ...
    for (const j of journals || []) {
      ...
      for (const l of lines || []) {
        totalDr += l.debit_amount;
        totalCr += l.credit_amount;
      }
    }
    ...
    checks.push({
      ...
      message: balanced
        ...
        : `Trial Balance out of balance: Dr ${totalDr.toFixed(2)}, Cr ${totalCr.toFixed(2)}`,
    ...
    checks.push({
      ...
      message:
        (unposted?.length || 0) > 0
          ? `${unposted!.length} unposted journal(s) found`
          : "All journals posted",
  ...
  async getIntegrityScore(
    entityId: string,
    periodId: string,
  ): Promise<FinancialIntegrityScore> {
  ...
  async getCloseChecklist(
    ...
    const items = [
      ...
      "Supplier Invoices Captured",
      "Receipts Allocated",
      "Payments Reconciled",
      "Arrears Reviewed",
      "Broker Commissions Reviewed",
      "VAT Reviewed",
      "Outstanding Approvals Reviewed",
  ...
  async canClosePeriod(
    ...
    const criticalCount = checks.filter(
      (c) => c.level === "critical" && !c.acknowledged,
    ).length;
    const warningCount = checks.filter(
      (c) => c.level === "warning" && !c.acknowledged,
    ).length;
