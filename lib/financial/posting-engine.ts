...
export class PostingEngine {
  async post(event: FinancialEvent): Promise<PostingResult> {
    ...
    if (!validation.valid) {
      logger.error("Financial event validation failed", {
    ...
    if (!template || !template.lines?.length)
      throw new Error(`No posting template for: ${event.business_event}`);

    const vatTreatment: VatCategory = event.vat_treatment || "standard";
    const { vatAmount, vatRate } = financialRulesEngine.calculateVat(
      event.amount,
      vatTreatment,
    );
    const context = financialRulesEngine.buildFormulaContext(
      event,
      vatAmount,
      vatRate,
    );
    ...
    const journal: Journal = {
      ...
      period_id: periodId,
      source_event: event.business_event,
    ...
    const settingsUsed: Record<string, any> = {
      vat_treatment: vatTreatment,
      vat_rate: vatRate,
    };

    for (const tl of template.lines) {
      if (!formulaEngine.evaluateCondition(tl.condition_formula, context))
        continue;

      const accountId = await financialRulesEngine.resolveAccountId(
        event.entity_id,
        tl.account_resolver,
      );
      if (!accountId)
        throw new Error(`Cannot resolve account: ${tl.account_resolver}`);
      ...
      const line: JournalLine = {
        ...
        vat_amount: lineVat,
        vat_rate: tl.vat_treatment === "standard" ? vatRate : 0,
        entity_id: event.entity_id,
        ...
        bank_account_id: event.dimensions?.bank_account_id || null,
      ...
      if (event.metadata?.override_reason)
        overridesApplied.push(event.metadata.override_reason);
    }
    ...
    if (Math.abs(totalDr - totalCr) > 0.01)
      throw new Error(`Journal unbalanced: Dr ${totalDr}, Cr ${totalCr}`);
    ...
    try {
      ...
      if (!postedJournalId) {
        throw new Error(
          "Atomic posting completed without returning journal ID",
        );
      }

      let persistedJournal = journal;

      if (postedJournalId !== journalId) {
        logger.info("Idempotent journal replay detected", {
          requestedJournalId: journalId,
          existingJournalId: postedJournalId,
          event: event.business_event,
          sourceId: event.metadata?.source_id,
        });
        ...
        if (existingJournalError || !existingJournal) {
          throw new Error(
            existingJournalError?.message ||
              `Existing journal ${postedJournalId} could not be loaded`,
          );
        }
        ...
        const { data: existingLines, error: existingLinesError } =
          await supabase
            .from("journal_lines")
            .select("*")
            .eq("journal_id", postedJournalId)
            .order("created_at", { ascending: true });

        if (existingLinesError) {
          throw new Error(
            `Existing journal lines could not be loaded: ${existingLinesError.message}`,
          );
        }
      ...
      await subLedgerEngine.postToSubLedgers(persistedJournal);

      await financialTimelineEngine.recordJournalLifecycle(
        postedJournalId,
        persistedJournal.entity_id,
        "posted",
      ...
      if (postedJournalId === journalId) {
        await publish("financial.journal.posted", {
          ...
          payload: {
            ...
            entityId: persistedJournal.entity_id,
            balanced: true,
          },
        });
      }
      ...
      logger.info("Journal posted successfully", {
        ...
        event: event.business_event,
        entityId: persistedJournal.entity_id,
        replay: postedJournalId !== journalId,
      });
