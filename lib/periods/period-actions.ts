...
// Period Governance — Orchestrates domain services. Reacts to business outcomes.
...
export interface PeriodActionResult {
  success: boolean; nextPeriod: string; message: string; validations: ValidationResult[];
}
...
export async function startBillingRun(entityId: string, statementPeriod: string): Promise<PeriodActionResult> {
  ...
  // Publish first — Revenue owns the billing lifecycle
  ...
  return { success: true, nextPeriod: statementPeriod, message: `Billing run requested for ${statementPeriod}`, validations };
}
...
export async function closeStatementPeriod(entityId: string, periodName: string): Promise<PeriodActionResult> {
  ...
  const validations: ValidationResult[] = [
    { check: 'invoices_generated', passed: billingStatus.completed, message: `${billingStatus.invoicesGenerated} invoices generated` },
    { check: 'cashbook_reconciled', passed: reconciliationStatus.balanced, message: reconciliationStatus.balanced ? 'Cash book reconciled' : `${reconciliationStatus.unreconciled} unreconciled` },
  ...
  const { error } = await supabase.from('financial_periods').update({ status: 'closed', closed_at: new Date().toISOString() }).eq('entity_id', entityId).eq('period_type', 'statement').eq('period_name', periodName);
  ...
  await supabase.from('financial_periods').insert({ entity_id: entityId, period_type: 'statement', period_name: nextPeriod, period_start: startDate, period_end: endDate, status: 'open' });
  ...
  return { success: true, nextPeriod, message: `Statement ${periodName} closed`, validations };
}
...
export async function closeFinancialPeriod(entityId: string, periodName: string): Promise<PeriodActionResult> {
  ...
  const validations: ValidationResult[] = [
    { check: 'statements_closed', passed: (openStatements?.length || 0) === 0, message: openStatements?.length ? `${openStatements.length} open statements` : 'All statements closed' },
    { check: 'trial_balance', passed: tbStatus.balanced, message: tbStatus.balanced ? 'TB balanced' : 'TB out of balance' },
    { check: 'cashbook_reconciled', passed: reconciliationStatus.balanced, message: reconciliationStatus.balanced ? 'Cash book reconciled' : `${reconciliationStatus.unreconciled} unreconciled` },
  ];
  if (!validations.every(v => v.passed)) return { success: false, nextPeriod: '', message: 'Close validations failed', validations };

  const { error } = await supabase.from('financial_periods').update({ status: 'closed', closed_at: new Date().toISOString() }).eq('entity_id', entityId).eq('period_type', 'financial').eq('period_name', periodName);
  if (error) return { success: false, nextPeriod: '', message: error.message, validations: [] };

  const { nextPeriod, startDate, endDate } = getNextPeriodWithDates(periodName);
  await supabase.from('financial_periods').insert({ entity_id: entityId, period_type: 'financial', period_name: nextPeriod, period_start: startDate, period_end: endDate, status: 'open' });
  ...
  return { success: true, nextPeriod, message: `Financial ${periodName} closed`, validations };
}
