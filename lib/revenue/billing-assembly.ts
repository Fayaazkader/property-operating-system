...
export const billingAssembly = {
  async assembleWorksheet(entityId: string, propertyId?: string): Promise<BillingWorksheet> {
    ...
    return { property_name: tenants, totalCharges: tenants.reduce((s, t) => s + t.total, 0), readyCount: tenants.filter(t => t.ready).length, warningCount: tenants.filter(t => !t.ready).length };
  },
