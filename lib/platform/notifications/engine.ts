...
export class NotificationEngine {
  ...
  initialize(): void {
    ...
    // Subscribe to notification.requested events
  ...
  async send(request: NotificationRequest): Promise<Notification> {
    ...
    // 3. Check preferences
    const userPrefs = await notificationPreferences.getPreferences(
      request.recipient,
  ...
  private async createNotification(
    ...
    if (skipReason) {
      notification.metadata = { skipReason };
    }
...
ENDOFFILEcat > lib/platform/notifications/engine.ts << 'ENDOFFILE'
// lib/platform/notifications/engine.ts
// Notification Engine — Core
