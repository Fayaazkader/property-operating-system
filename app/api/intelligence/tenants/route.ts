...
export async function GET(request: NextRequest) {
  ...
  // Get all tenants directly for testing
  const from = page * pageSize;
  const to = from + pageSize - 1;

  const { data: tenantsData, count } = await supabase
    .from("tenants")
    .select("id, tenant_name, code, company_registration, email, entity_id, whatsapp_enabled", { count: "exact" })
    .range(from, to)
    .order("tenant_name");

  if (!tenantsData) return NextResponse.json({ tenants: [], total: 0 });
  ...
  const { data: chargesData } = await supabase
    .from("charges")
    .select("tenant_id, amount_incl_vat")
    .in("tenant_id", tenantIds);

  const { data: paymentsData } = await supabase
    .from("bank_transactions")
    .select("matched_tenant_id, transaction_amount")
    .in("matched_tenant_id", tenantIds);
  ...
  const enriched = tenantsData.map((t: any) => {
    ...
    return {
      ...
      code: t.code || null,
      company_registration: t.company_registration || null,
      email: t.email,
      entity_id: t.entity_id,
      entity_name: entities?.find(e => e.id === t.entity_id)?.entity_name || "Unknown",
      property_id: lease?.property_id || null,
      property_name: (lease as any)?.properties?.property_name || null,
      current_lease: lease?.lease_id || null,
      lease_status: lease?.lease_status || null,
      monthly_rental: lease?.monthly_rental || null,
  ...
  return NextResponse.json({ tenants: enriched, total: count || 0, page, pageSize });
}
