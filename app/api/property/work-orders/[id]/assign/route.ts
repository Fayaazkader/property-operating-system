// app/api/property/work-orders/[id]/assign/route.ts
...
export async function POST(req: NextRequest, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    const validation = validateRequired(body, [{ field: 'supplier_id', label: 'Supplier ID' }]);
    if (!validation.valid) {
      return validationErrorResponse(validation.errors, platformContext.correlationId);
    }
    const result = await propertyOrchestrator.assignWorkOrder(id, body.supplier_id);
    if (result.error) {
