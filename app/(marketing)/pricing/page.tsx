...
const TIERS = [
  ...
  { leases: '1–100', price: '2,995', users: '4', messages: '1,000', desc: 'For growing portfolios' },
...
const ALL_FEATURES = [
  'Full AssetFlow Platform',
  'Revenue Operations',
  'Financial Platform (GL, AP, AR)',
  'Property Operations',
        ...
        <Container>
          ...
          <div className="grid gap-4 md:grid-cols-5 mb-20">
            {TIERS.map((tier) => (
              ...
              >
                {tier.featured && (
                  <p className="text-[10px] font-medium text-amber-400 mb-3 uppercase tracking-wider">Most Popular</p>
                )}
                ...
                <p className="mt-3 text-4xl font-light tracking-tight text-white">
                  R{tier.price}
                  <span className="text-sm text-zinc-500 font-light">/mo</span>
                </p>
                ...
                <Link
                  ...
                  className={`mt-5 block w-full rounded-full py-2.5 text-xs font-medium transition-all duration-300 ${
                    tier.featured
                      ? 'bg-white text-black hover:bg-zinc-200'
                      : 'border border-white/[0.08] text-white hover:border-white/20'
                  }`}
                >
                  {tier.featured ? 'Book a Demo' : 'Get Started'}
                </Link>
              </div>
            ))}
          </div>
          ...
          <div className="text-center mb-4">
            <p className="text-sm text-zinc-500 font-light">
              1,000+ leases?{' '}
              <Link href={ROUTES.PUBLIC.CONTACT} className="text-white underline underline-offset-4 hover:text-zinc-300">
                Contact us
              </Link>{' '}
