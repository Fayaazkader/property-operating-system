...
export default function PricingPage() {
  return (
    <Section id="pricing" className="pt-32 md:pt-48 pb-24">
      <Container>
        <div className="text-center mb-20">
          <p className="text-xs uppercase tracking-[0.3em] text-amber-400/80 mb-6 font-medium">Pricing</p>
        ...
        <div className="text-center mb-4">
          <p className="text-sm text-zinc-500 font-light">1,000+ leases? <Link href="/contact" className="text-white underline underline-offset-4 hover:text-zinc-300">Contact us</Link> for enterprise pricing.</p>
          ...
          <div className="grid gap-3 md:grid-cols-3 max-w-3xl mx-auto">
            {ALL_FEATURES.map((feature) => (
              <div key={feature} className="flex items-center gap-2 text-sm text-zinc-400 font-light">
                <span className="text-amber-400/60 text-xs">✓</span> {feature}
              </div>
            ))}
          </div>
        </div>
