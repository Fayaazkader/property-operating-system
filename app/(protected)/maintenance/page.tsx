...
import { Search, Plus, ArrowRight, Zap, Clock, Shield, MapPin } from 'lucide-react';
...
export default function MaintenanceCommand() {
  ...
  useEffect(() => {
    async function load() {
      ...
      pending.slice(0, 2).forEach(i => feed.push({ 
        type: 'urgent', issue: i, 
        context: 'Awaiting work order',
        action: 'Create WO'
      }));

      setAttention(feed.slice(0, 4));
      setLoading(false);
  ...
  // Build timeline: today's activity + upcoming
  const timeline = [
    ...issues
      ...
      .slice(0, 8)
      .map(i => ({
        time: new Date(i.created_at).toLocaleTimeString('en-ZA', { hour: '2-digit', minute: '2-digit' }),
        text: i.title,
        type: i.priority === 'emergency' ? 'emergency' : 'activity',
        date: new Date(i.created_at).toDateString() === new Date().toDateString() ? 'Today' : new Date(i.created_at).toLocaleDateString('en-ZA', { day: 'numeric', month: 'short' }),
      })),
    { time: '11:00', text: 'Generator Service', type: 'upcoming', date: 'Tomorrow' },
    { time: '09:00', text: 'Lift Inspection', type: 'upcoming', date: 'Friday' },
    { time: '08:00', text: 'Fire Audit', type: 'upcoming', date: 'Next Week' },
  ];
  ...
  return (
    <div className="flex h-full">
      <div className="flex-1 p-8 lg:p-10 overflow-y-auto space-y-10">
        ...
        {/* OPERATIONS PULSE */}
        <div className={`rounded-2xl border border-white/[0.04] bg-gradient-to-r from-white/[0.01] via-transparent to-white/[0.01] px-6 py-4 transition-all duration-700 ${visible ? 'opacity-100' : 'opacity-0 translate-y-2'}`}>
        ...
        {/* HEADER */}
        <div className={`transition-all duration-700 delay-100 ${visible ? 'opacity-100' : 'opacity-0 translate-y-2'}`}>
        ...
        {/* SEARCH */}
        <div className={`relative transition-all duration-700 delay-200 ${visible ? 'opacity-100' : 'opacity-0 translate-y-2'}`}>
        ...
        {/* CONTEXTUAL ACTIONS */}
        <div className={`flex gap-2 flex-wrap transition-all duration-700 delay-300 ${visible ? 'opacity-100' : 'opacity-0 translate-y-2'}`}>
        ...
        {/* ATTENTION FEED */}
        {attention.length > 0 && (
          <div className={`space-y-3 transition-all duration-700 delay-400 ${visible ? 'opacity-100' : 'opacity-0 translate-y-2'}`}>
            <p className="text-[10px] uppercase tracking-[0.2em] text-zinc-600">Needs Attention</p>
            <div className="space-y-2">
              {attention.map((item, i) => (
                <div key={i} className="group flex items-center justify-between rounded-xl border border-white/[0.04] bg-white/[0.01] p-5 hover:bg-white/[0.02] hover:border-white/[0.06] transition-all duration-300"
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <div className="flex items-center gap-2">
                        ...
                        {item.issue.property_id && (
                          <span className="flex items-center gap-1 text-[10px] text-zinc-600">
                            <MapPin className="w-2.5 h-2.5" /> Property
                          </span>
                        )}
                      </div>
        ...
        {/* PIPELINE */}
        <div className={`space-y-3 transition-all duration-700 delay-500 ${visible ? 'opacity-100' : 'opacity-0 translate-y-2'}`}>
          ...
          <div className="grid grid-cols-5 gap-2">
            {stages.map((stage, idx) => {
              ...
              const urgentCount = stageIssues.filter(i => i.priority === 'emergency' || i.priority === 'urgent').length;
              return (
                <div key={stage.key} 
                  className="group rounded-xl border border-white/[0.04] bg-white/[0.01] p-4 hover:bg-white/[0.02] transition-all duration-300 cursor-pointer"
                  ...
                  {urgentCount > 0 && (
                    <div className="flex gap-1 mt-2">
                      {Array.from({ length: Math.min(urgentCount, 4) }).map((_, i) => (
                        <div key={i} className={`w-1 h-4 rounded-full ${stage.color} opacity-60`} />
                      ))}
                    </div>
                  )}
                  {idx < stages.length - 1 && (
      ...
      {/* RIGHT PANEL — Continuous Timeline */}
            ...
            {timeline.map((item, i) => (
              <div key={i} className="relative pl-5">
                <div className={`absolute left-0 top-1.5 w-2.5 h-2.5 rounded-full border-2 border-black ${
                  item.type === 'emergency' ? 'bg-red-400' : 
                  item.type === 'upcoming' ? 'bg-zinc-600' : 'bg-zinc-500'
                }`} />
                <p className="text-[10px] text-zinc-600">{item.time} · {item.date}</p>
                <p className="text-xs text-white font-light mt-0.5">{item.text}</p>
              </div>
            ))}
          </div>
        </div>
