    ...
    <div className="p-8 max-w-7xl mx-auto space-y-6">
                  ...
                  return (
                    <tr key={inv.id} className="border-b border-white/[0.03]">
                      ...
                      <td className="py-2.5 px-4 text-right">
                        <input value={payAmount} onChange={(e) => handleEditAmount(inv.id, e.target.value)} onBlur={() => saveAmount(inv.id)} className="w-24 text-right rounded border border-white/[0.08] bg-[var(--bg-secondary)] px-2 py-1 text-xs text-white outline-none focus:border-white/20" />
                      </td>
      ...
      {showCreateBatch && (
        <>
          ...
          <div className="fixed inset-4 z-50 flex items-center justify-center p-4"><div className="bg-[var(--bg-primary)] border border-white/[0.08] rounded-2xl p-6 w-full max-w-md"><div className="flex justify-between items-center mb-4"><p className="text-sm font-medium text-white">Create Payment Batch</p><button onClick={() => setShowCreateBatch(false)} className="text-zinc-500 hover:text-white">✕</button></div><div className="space-y-3"><p className="text-xs text-zinc-400">{approvedRequests.length} approved · R{approvedRequests.reduce((s: number, r: any) => s + r.amount, 0).toLocaleString()}</p><select value={batchBankAccount} onChange={(e) => setBatchBankAccount(e.target.value)} className="w-full rounded-lg border border-white/[0.08] bg-[var(--bg-secondary)] px-3 py-2.5 text-sm text-white outline-none"><option value="">Select bank account...</option>{bankAccounts.map(b => (<option key={b.id} value={b.id}>{b.bank_name} — {b.account_number}</option>))}</select><input value={batchReference} onChange={(e) => setBatchReference(e.target.value)} placeholder="Reference (e.g. July Supplier Run)" className="w-full rounded-lg border border-white/[0.08] bg-[var(--bg-secondary)] px-3 py-2.5 text-sm text-white outline-none" /><button onClick={createBatch} className="w-full rounded-lg bg-white py-2.5 text-sm font-medium text-black hover:bg-gray-100">Create Batch</button></div></div></div>
        </>
      )}
    </div>
  );
