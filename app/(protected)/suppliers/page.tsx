    ...
    <div className="flex h-full">
      ...
      <div className="flex-1 p-8 overflow-y-auto">
        <div className="flex items-center justify-between mb-6">
          ...
          <button onClick={() => setShowBulk(true)} className="rounded-lg border border-white/[0.08] px-4 py-2.5 text-xs font-medium text-white hover:border-white/20">Bulk Upload</button>        </div>
      ...
      {/* BULK UPLOAD MODAL */}
      {showBulk && (<Modal title="Bulk Upload" onClose={() => setShowBulk(false)}><div className="border-2 border-dashed border-white/[0.1] rounded-xl p-8 text-center space-y-3"><p className="text-sm text-zinc-400">Drag and drop PDFs, images, or scans here</p><p className="text-xs text-zinc-600">OCR will process automatically · Drafts created for review</p><input type="file" multiple accept=".pdf,.jpg,.png,.jpeg" className="text-xs text-zinc-500" /><p className="text-xs text-zinc-500 mt-2">Files will appear in Approval Queue after processing</p></div></Modal>)}
      {/* SUPPLIER FORM MODAL */}
