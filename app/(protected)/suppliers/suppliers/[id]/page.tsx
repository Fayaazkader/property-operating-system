    ...
    <div className="space-y-6">
      ...
      {showEdit && (
        <>
          ...
          <div className="fixed inset-4 z-50 flex items-center justify-center p-4">
            <div className="bg-[var(--bg-primary)] border border-white/[0.08] rounded-2xl w-full max-w-4xl max-h-[85vh] flex flex-col">
              ...
              <div className="flex-1 overflow-y-auto p-6">
                <div className="grid grid-cols-2 gap-8">
                  <div className="space-y-4">
                    ...
                    <input value={form.supplier_name || ''} onChange={(e) => setForm({ ...form, supplier_name: e.target.value })} placeholder="Supplier Name *" className="w-full rounded-lg border border-white/[0.08] bg-[var(--bg-secondary)] px-3 py-2.5 text-sm text-white outline-none focus:border-white/10 focus:ring-0" />
                    <div className="grid grid-cols-2 gap-2">
                      <input value={form.trading_name || ''} onChange={(e) => setForm({ ...form, trading_name: e.target.value })} placeholder="Trading Name" className="rounded-lg border border-white/[0.08] bg-[var(--bg-secondary)] px-3 py-2.5 text-sm text-white outline-none focus:border-white/10 focus:ring-0" />
                      <input value={form.registered_name || ''} onChange={(e) => setForm({ ...form, registered_name: e.target.value })} placeholder="Registered Name" className="rounded-lg border border-white/[0.08] bg-[var(--bg-secondary)] px-3 py-2.5 text-sm text-white outline-none focus:border-white/10 focus:ring-0" />
                    </div>
                    <div className="grid grid-cols-3 gap-2">
                      <input value={form.registration_number || ''} onChange={(e) => setForm({ ...form, registration_number: e.target.value })} placeholder="Reg #" className="rounded-lg border border-white/[0.08] bg-[var(--bg-secondary)] px-3 py-2.5 text-sm text-white outline-none focus:border-white/10 focus:ring-0" />
                      <input value={form.vat_number || ''} onChange={(e) => setForm({ ...form, vat_number: e.target.value })} placeholder="VAT #" className="rounded-lg border border-white/[0.08] bg-[var(--bg-secondary)] px-3 py-2.5 text-sm text-white outline-none focus:border-white/10 focus:ring-0" />
                      <input value={form.tax_number || ''} onChange={(e) => setForm({ ...form, tax_number: e.target.value })} placeholder="Tax #" className="rounded-lg border border-white/[0.08] bg-[var(--bg-secondary)] px-3 py-2.5 text-sm text-white outline-none focus:border-white/10 focus:ring-0" />
                    </div>
                    <select value={form.category || ''} onChange={(e) => setForm({ ...form, category: e.target.value })} className="w-full rounded-lg border border-white/[0.08] bg-[var(--bg-secondary)] px-3 py-2.5 text-sm text-white outline-none focus:border-white/10 focus:ring-0">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      <select value={form.payment_method || 'eft'} onChange={(e) => setForm({ ...form, payment_method: e.target.value })} className="rounded-lg border border-white/[0.08] bg-[var(--bg-secondary)] px-3 py-2.5 text-sm text-white outline-none focus:border-white/10 focus:ring-0">
                        <option value="eft">EFT</option><option value="debit_order">Debit Order</option><option value="cash">Cash</option>
                      </select>
                      <input value={form.default_payment_terms || '30'} onChange={(e) => setForm({ ...form, default_payment_terms: e.target.value })} placeholder="Payment Terms (days)" className="rounded-lg border border-white/[0.08] bg-[var(--bg-secondary)] px-3 py-2.5 text-sm text-white outline-none focus:border-white/10 focus:ring-0" />
                    </div>
                  </div>
                  <div className="space-y-4">
                    <p className="text-[10px] uppercase tracking-wider text-zinc-500">Contact</p>
                    <div className="grid grid-cols-2 gap-2">
                      <input value={form.contact_person || ''} onChange={(e) => setForm({ ...form, contact_person: e.target.value })} placeholder="Contact Person" className="rounded-lg border border-white/[0.08] bg-[var(--bg-secondary)] px-3 py-2.5 text-sm text-white outline-none focus:border-white/10 focus:ring-0" />
                      <input value={form.accounts_contact || ''} onChange={(e) => setForm({ ...form, accounts_contact: e.target.value })} placeholder="Accounts Contact" className="rounded-lg border border-white/[0.08] bg-[var(--bg-secondary)] px-3 py-2.5 text-sm text-white outline-none focus:border-white/10 focus:ring-0" />
                    </div>
                    <div className="grid grid-cols-2 gap-2">
                      <input value={form.email || ''} onChange={(e) => setForm({ ...form, email: e.target.value })} placeholder="Email" className="rounded-lg border border-white/[0.08] bg-[var(--bg-secondary)] px-3 py-2.5 text-sm text-white outline-none focus:border-white/10 focus:ring-0" />
                      <input value={form.phone || ''} onChange={(e) => setForm({ ...form, phone: e.target.value })} placeholder="Phone" className="rounded-lg border border-white/[0.08] bg-[var(--bg-secondary)] px-3 py-2.5 text-sm text-white outline-none focus:border-white/10 focus:ring-0" />
                    </div>
                    <p className="text-[10px] uppercase tracking-wider text-zinc-500 pt-4">Banking</p>
                    <div className="grid grid-cols-3 gap-2">
                      <input value={form.bank_name || ''} onChange={(e) => setForm({ ...form, bank_name: e.target.value })} placeholder="Bank" className="rounded-lg border border-white/[0.08] bg-[var(--bg-secondary)] px-3 py-2.5 text-sm text-white outline-none focus:border-white/10 focus:ring-0" />
                      <input value={form.bank_account || ''} onChange={(e) => setForm({ ...form, bank_account: e.target.value })} placeholder="Account" className="rounded-lg border border-white/[0.08] bg-[var(--bg-secondary)] px-3 py-2.5 text-sm text-white outline-none focus:border-white/10 focus:ring-0" />
                      <input value={form.bank_branch || ''} onChange={(e) => setForm({ ...form, bank_branch: e.target.value })} placeholder="Branch" className="rounded-lg border border-white/[0.08] bg-[var(--bg-secondary)] px-3 py-2.5 text-sm text-white outline-none focus:border-white/10 focus:ring-0" />
                    </div>
      ...
      {showLink && (
        <>
          ...
          <div className="fixed inset-4 z-50 flex items-center justify-center p-4"><div className="bg-[var(--bg-primary)] border border-white/[0.08] rounded-2xl p-6 w-full max-w-md"><div className="flex justify-between items-center mb-4"><p className="text-sm font-medium text-white">Link New Service</p><button onClick={() => setShowLink(false)} className="text-zinc-500 hover:text-white">✕</button></div><div className="space-y-3"><select value={linkForm.property_id} onChange={(e) => setLinkForm({ ...linkForm, property_id: e.target.value })} className="w-full rounded-lg border border-white/[0.08] bg-[var(--bg-secondary)] px-3 py-2.5 text-sm text-white outline-none"><option value="">Select property...</option>{properties.map(p => (<option key={p.id} value={p.id}>{p.property_name}</option>))}</select><input value={linkForm.account_number} onChange={(e) => setLinkForm({ ...linkForm, account_number: e.target.value })} placeholder="Reference Number *" className="w-full rounded-lg border border-white/[0.08] bg-[var(--bg-secondary)] px-3 py-2.5 text-sm text-white outline-none" /><input value={linkForm.account_name} onChange={(e) => setLinkForm({ ...linkForm, account_name: e.target.value })} placeholder="Account Name (e.g. Electricity)" className="w-full rounded-lg border border-white/[0.08] bg-[var(--bg-secondary)] px-3 py-2.5 text-sm text-white outline-none" /><div className="grid grid-cols-2 gap-2"><input value={linkForm.default_gl} onChange={(e) => setLinkForm({ ...linkForm, default_gl: e.target.value })} placeholder="Default GL" className="rounded-lg border border-white/[0.08] bg-[var(--bg-secondary)] px-3 py-2.5 text-sm text-white outline-none" /><select value={linkForm.default_vat} onChange={(e) => setLinkForm({ ...linkForm, default_vat: e.target.value })} className="rounded-lg border border-white/[0.08] bg-[var(--bg-secondary)] px-3 py-2.5 text-sm text-white outline-none"><option value="15">VAT 15%</option><option value="0">No VAT</option></select></div><button onClick={handleLinkService} className="w-full rounded-lg bg-white py-2.5 text-sm font-medium text-black hover:bg-gray-100">Save Service</button></div></div></div>
        </>
      )}
    </div>
  );
