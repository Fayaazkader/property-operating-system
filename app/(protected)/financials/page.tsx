...
export default function FinancialWorkspacePage() {
  ...
  async function loadBalanceSheet() {
    ...
    if (!tb) return null;
    const assets = tb.rows.filter((r: any) => r.account_type === 'asset');
    const liabilities = tb.rows.filter((r: any) => r.account_type === 'liability');
    const equity = tb.rows.filter((r: any) => r.account_type === 'equity');
    const totalAssets = assets.reduce((s: number, r: any) => s + r.net_balance, 0);
    const totalLiabilities = liabilities.reduce((s: number, r: any) => s + Math.abs(r.net_balance), 0);
    const totalEquity = equity.reduce((s: number, r: any) => s + Math.abs(r.net_balance), 0);
  ...
  async function loadCashFlow() {
    // Simplified cash flow from bank GL entries
    const { data: bankAccounts } = await supabase.from('bank_accounts').select('id').eq('entity_id', entityId);
    if (!bankAccounts?.length) return null;
    const bankIds = bankAccounts.map(b => b.id);
    const { data: lines } = await supabase.from('journal_lines').select('debit_amount, credit_amount, description, journals!inner(source_event, period_id)').in('account_id', bankIds).eq('journals.period_id', periodId);
    const operating = (lines || []).filter((l: any) => (l.journals as any)?.source_event?.includes('rental') || (l.journals as any)?.source_event?.includes('receipt') || (l.journals as any)?.source_event?.includes('supplier'));
    const netCash = (lines || []).reduce((s: number, l: any) => s + (l.debit_amount || 0) - (l.credit_amount || 0), 0);
    return { operating: operating.map((l: any) => ({ description: l.description, amount: (l.debit_amount || 0) - (l.credit_amount || 0) })), net_cash_flow: netCash, opening_cash: 0, closing_cash: netCash };
  }
  ...
  async function loadBudget() {
    ...
    return tb.rows.filter((r: any) => r.account_type === 'income' || r.account_type === 'expense').map((r: any) => ({
      account: `${r.gl_code} - ${r.account_name}`,
      actual: Math.abs(r.net_balance),
      budgeted: budgetMap.get(r.account_id) || 0,
      variance: Math.abs(r.net_balance) - (budgetMap.get(r.account_id) || 0),
    }));
  }
  ...
  // Filter trial balance by search
    ...
    <div className="flex h-full">
      ...
      <div className="flex-1 p-8 overflow-y-auto">
        ...
        <div className="flex items-center gap-3 mb-6 flex-wrap">
          ...
          <select value={filterProperty} onChange={(e) => setFilterProperty(e.target.value)} className="rounded-lg border border-white/[0.08] bg-zinc-900 px-3 py-2 text-sm text-white outline-none">
        ...
        {!loading && !error && (
          <>
            ...
            {section === 'trial-balance' && <TrialBalancePanel rows={filteredTB} totalDr={data.trialBalance?.totalDr} totalCr={data.trialBalance?.totalCr} balanced={data.trialBalance?.balanced} />}
  ...
  return (
    <div className="space-y-6">
      <div className="grid gap-4 md:grid-cols-4">
        ...
        <div className="rounded-xl border border-white/[0.06] bg-white/[0.01] p-4"><p className="text-[10px] uppercase tracking-[0.2em] text-zinc-600 mb-1">Integrity</p><p className="text-xl font-light text-white">{closeData?.integrity_score?.percentage || 0}%</p></div>
        <div className="rounded-xl border border-white/[0.06] bg-white/[0.01] p-4"><p className="text-[10px] uppercase tracking-[0.2em] text-zinc-600 mb-1">Critical</p><p className={`text-xl font-light ${!closeData || closeData.critical_count === 0 ? 'text-emerald-400' : 'text-red-400'}`}>{closeData?.critical_count || 0}</p></div>
        <div className="rounded-xl border border-white/[0.06] bg-white/[0.01] p-4"><p className="text-[10px] uppercase tracking-[0.2em] text-zinc-600 mb-1">Can Close</p><p className={`text-xl font-light ${closeData?.can_close ? 'text-emerald-400' : 'text-amber-400'}`}>{closeData?.can_close ? 'Yes' : 'Not yet'}</p></div>
      </div>
    </div>
  );
}

function TrialBalancePanel({ rows, totalDr, totalCr, balanced }: any) {
  ...
  return (
    <div className="rounded-xl border border-white/[0.06] overflow-hidden">
      <table className="w-full text-sm">
        ...
        <tfoot><tr className="bg-white/[0.02]"><td colSpan={2} className="py-3 px-4 text-zinc-400 text-[11px] uppercase">Totals</td><td className="py-3 px-4 text-right text-white font-medium tabular-nums">R{totalDr.toLocaleString()}</td><td className="py-3 px-4 text-right text-white font-medium tabular-nums">R{totalCr.toLocaleString()}</td></tr></tfoot>
      </table>
  ...
  return (
    <div className="space-y-6">
      <div className="grid gap-6 md:grid-cols-2">
        <div className="rounded-xl border border-white/[0.06] overflow-hidden">
          <div className="px-5 py-3 border-b border-white/[0.06] bg-white/[0.02]"><p className="text-[11px] font-medium text-zinc-500 uppercase">Assets</p></div>
          {data.assets.map((a: any, i: number) => (<div key={i} className="flex justify-between px-5 py-2.5 border-b border-white/[0.03]"><span className="text-sm text-zinc-300 font-light">{a.account}</span><span className="text-sm text-white font-light tabular-nums">R{a.amount.toLocaleString()}</span></div>))}
          <div className="flex justify-between px-5 py-3 bg-white/[0.01]"><span className="text-sm text-white font-medium">Total Assets</span><span className="text-sm text-white font-medium">R{data.total_assets.toLocaleString()}</span></div>
        </div>
        <div className="space-y-6">
          <div className="rounded-xl border border-white/[0.06] overflow-hidden">
            <div className="px-5 py-3 border-b border-white/[0.06] bg-white/[0.02]"><p className="text-[11px] font-medium text-zinc-500 uppercase">Liabilities</p></div>
            {data.liabilities.map((l: any, i: number) => (<div key={i} className="flex justify-between px-5 py-2.5 border-b border-white/[0.03]"><span className="text-sm text-zinc-300 font-light">{l.account}</span><span className="text-sm text-white font-light tabular-nums">R{l.amount.toLocaleString()}</span></div>))}
            <div className="flex justify-between px-5 py-3 bg-white/[0.01]"><span className="text-sm text-white font-medium">Total Liabilities</span><span className="text-sm text-white font-medium">R{data.total_liabilities.toLocaleString()}</span></div>
          </div>
          <div className="rounded-xl border border-white/[0.06] overflow-hidden">
            <div className="px-5 py-3 border-b border-white/[0.06] bg-white/[0.02]"><p className="text-[11px] font-medium text-zinc-500 uppercase">Equity</p></div>
            {data.equity.map((e: any, i: number) => (<div key={i} className="flex justify-between px-5 py-2.5 border-b border-white/[0.03]"><span className="text-sm text-zinc-300 font-light">{e.account}</span><span className="text-sm text-white font-light tabular-nums">R{e.amount.toLocaleString()}</span></div>))}
            <div className="flex justify-between px-5 py-3 bg-white/[0.01]"><span className="text-sm text-white font-medium">Total Equity</span><span className="text-sm text-white font-medium">R{data.total_equity.toLocaleString()}</span></div>
          </div>
        </div>
      </div>
      <div className="rounded-xl border border-white/[0.06] bg-white/[0.02] p-4 text-center">
        <span className={`text-sm ${data.balanced ? 'text-emerald-400' : 'text-red-400'}`}>
          {data.balanced ? '✓ Balance Sheet Balanced' : '✗ Balance Sheet does not balance'}
        </span>
      </div>
    </div>
  );
...
function CashFlowPanel({ data }: any) {
  ...
  return (
    <div className="space-y-6">
      <div className="rounded-xl border border-white/[0.06] overflow-hidden">
        <div className="px-5 py-3 border-b border-white/[0.06] bg-white/[0.02]"><p className="text-[11px] font-medium text-zinc-500 uppercase">Operating Activities</p></div>
        {data.operating.map((o: any, i: number) => (<div key={i} className="flex justify-between px-5 py-2.5 border-b border-white/[0.03]"><span className="text-sm text-zinc-300 font-light">{o.description}</span><span className={`text-sm font-light tabular-nums ${o.amount >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>R{o.amount.toLocaleString()}</span></div>))}
      </div>
