      ...
      <div className="flex-1 p-8 overflow-y-auto">
        ...
        <div className="flex items-center gap-3 mb-6 flex-wrap justify-between">
          <div className="flex items-center gap-3 flex-wrap">
            <select value={periodId} onChange={(e) => setPeriodId(e.target.value)} className="rounded-lg border border-white/[0.08] bg-[var(--bg-secondary)] px-3 py-2 text-sm text-white outline-none">
            ...
            <select value={filterProperty} onChange={(e) => setFilterProperty(e.target.value)} className="rounded-lg border border-white/[0.08] bg-[var(--bg-secondary)] px-3 py-2 text-sm text-white outline-none"><option value="">All Properties</option>{properties.map(p => (<option key={p.id} value={p.id}>{p.property_name}</option>))}</select>
            <div className="flex items-center gap-1">
              <input value={searchGL} onChange={(e) => setSearchGL(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleGlobalSearch()} placeholder="Search everything..." className="rounded-lg border border-white/[0.08] bg-[var(--bg-secondary)] px-3 py-2 text-sm text-white outline-none w-56" />
...
function JournalDrawer({ journal, onClose }: any) {
  return (<><div className="fixed inset-0 z-40 bg-black/60 backdrop-blur-sm" onClick={onClose} /><div className="fixed right-0 top-0 h-full w-[520px] z-50 bg-[var(--bg-primary)] border-l border-white/[0.08] shadow-2xl overflow-y-auto"><div className="p-6"><div className="flex justify-between items-center mb-6"><div><p className="text-sm font-medium text-white">{journal.journal_number}</p><p className="text-xs text-zinc-500 mt-0.5">{journal.description}</p></div><button onClick={onClose} className="text-zinc-500 hover:text-white text-lg">✕</button></div>
