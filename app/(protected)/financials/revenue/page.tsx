...
export default function RevenueOperationsPage() {
  ...
  useEffect(() => {
    async function init() {
      ...
      const { count: draftCount } = await supabase.from('manual_charges').select('*', { count: 'exact', head: true }).eq('entity_id', eid).eq('status', 'draft');
      if ((draftCount || 0) > 0) items.push({ type: 'draft_charges', count: draftCount || 0, label: 'Draft Manual Charges', action: 'Review' });
      const { count: interestDraftCount } = await supabase.from('interest_charges').select('*', { count: 'exact', head: true }).eq('entity_id', eid).eq('status', 'draft');
  ...
  async function handlePreview() {
    ...
    setLoading(true);
    try {
      const worksheet = await billingAssembly.assembleWorksheet(entityId, filterProperty || undefined);
      let tenants = worksheet.tenants;
      if (searchQuery) { const q = searchQuery.toLowerCase(); tenants = tenants.filter(t => t.tenantName.toLowerCase().includes(q) || t.propertyName.toLowerCase().includes(q)); }
  ...
  async function handleSendBilling() {
    ...
    for (let i = 0; i < ready.length; i++) {
      ...
      try {
        ...
        delivered++;
      } catch (err) { console.error('Send failed for', t.tenantName, err); failed++; }
    }
    await billingAssembly.saveSnapshot({ entityId, period: currentPeriod, propertyId: filterProperty, propertyName: billingTenants[0]?.propertyName || '', tenantCount: ready.length, invoicesGenerated: delivered, statementsGenerated: delivered, emailsDelivered: delivered, whatsappDelivered: Math.floor(delivered * 0.8), failed });
  ...
  function handleTenantSearch(q: string) { setChargeSearchQuery(q); if (!q) { setFilteredTenants(tenants); return; } setFilteredTenants(tenants.filter(t => t.tenant_name.toLowerCase().includes(q.toLowerCase()))); }
    ...
    <div className="p-8 max-w-7xl mx-auto space-y-8">
      ...
      {attentionItems.length > 0 && (
        <div className="rounded-xl border border-amber-500/20 bg-amber-500/5 p-4">
          ...
          <div className="space-y-2">{attentionItems.map((item, i) => (<div key={i} className="flex items-center justify-between text-sm"><span className="text-zinc-300 font-light">{item.count} {item.label}</span><button className="text-xs text-amber-400 hover:text-amber-300">{item.action} →</button></div>))}</div>
        </div>
      )}
      ...
      <div className="space-y-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex gap-2">
            <input value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handlePreview()} placeholder="Search tenant or property..." className="w-64 rounded-lg border border-white/[0.08] bg-zinc-900 px-3 py-2 text-xs text-white outline-none focus:border-white/20" />
            <select value={filterProperty} onChange={(e) => setFilterProperty(e.target.value)} className="rounded-lg border border-white/[0.08] bg-zinc-900 px-3 py-2 text-xs text-white outline-none"><option value="">All Properties</option>{properties.map(p => (<option key={p.id} value={p.id}>{p.property_name}</option>))}</select>
          </div>
          ...
          <button onClick={handleSendBilling} disabled={billingTenants.filter(t => t.ready).length === 0} className="rounded-lg bg-white px-4 py-2.5 text-xs font-medium text-black hover:bg-gray-100 disabled:opacity-40">Send Billing</button>
          <button onClick={() => setShowSnapshots(true)} className="rounded-lg border border-white/[0.08] px-4 py-2.5 text-xs font-medium text-white hover:border-white/20">History</button>
        </div>
      </div>
      ...
      {showPreview && (
        <>
          <div className="fixed inset-0 z-40 bg-black/60 backdrop-blur-sm" onClick={() => setShowPreview(false)} />
          <div className="fixed inset-4 z-50 overflow-y-auto flex items-start justify-center p-4">
            <div className="bg-zinc-950 border border-white/[0.08] rounded-2xl w-full max-w-5xl max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
              <div className="sticky top-0 bg-zinc-950 border-b border-white/[0.06] px-6 py-4 flex justify-between items-center rounded-t-2xl">
                <div>
                  <p className="text-sm font-medium text-white">Billing Preview — {currentPeriod}</p>
                  <p className="text-xs text-zinc-500">{billingTenants.length} tenants · R{billingTenants.reduce((s, t) => s + t.total, 0).toLocaleString()} total</p>
                </div>
                <button onClick={() => setShowPreview(false)} className="text-zinc-500 hover:text-white">Close ✕</button>
              </div>
              <div className="p-6">
                {billingTenants.length === 0 ? (
                  <p className="text-sm text-zinc-500 text-center py-8">No billing data found.</p>
                ) : (
                  <table className="w-full text-sm">
                    <thead><tr className="border-b border-white/[0.06]"><th className="text-left py-2.5 px-4 text-[10px] font-medium text-zinc-500 uppercase">Tenant</th><th className="text-center py-2.5 px-3 text-[10px] font-medium text-zinc-500 uppercase w-14">Rent</th><th className="text-center py-2.5 px-3 text-[10px] font-medium text-zinc-500 uppercase w-14">Utils</th><th className="text-center py-2.5 px-3 text-[10px] font-medium text-zinc-500 uppercase w-14">Manual</th><th className="text-center py-2.5 px-3 text-[10px] font-medium text-zinc-500 uppercase w-14">Int.</th><th className="text-center py-2.5 px-3 text-[10px] font-medium text-zinc-500 uppercase w-14">Fee</th><th className="text-center py-2.5 px-3 text-[10px] font-medium text-zinc-500 uppercase w-10">Docs</th><th className="text-center py-2.5 px-3 text-[10px] font-medium text-zinc-500 uppercase w-16">Status</th><th className="text-right py-2.5 px-4 text-[10px] font-medium text-zinc-500 uppercase w-28">Total</th></tr></thead>
                    <tbody>{billingTenants.map(t => (<tr key={t.tenantId} className="border-b border-white/[0.03] hover:bg-white/[0.01]"><td className="py-2.5 px-4 text-white font-light text-xs">{t.tenantName}<br /><span className="text-[10px] text-zinc-500">{t.propertyName}</span></td><td className="py-2.5 px-3 text-center text-xs">{t.charges.some(c => c.source === 'lease') ? '✓' : '—'}</td><td className="py-2.5 px-3 text-center text-xs">{t.charges.some(c => c.source === 'utility') ? '✓' : '—'}</td><td className="py-2.5 px-3 text-center text-xs">{t.charges.filter(c => c.source === 'manual').length || '—'}</td><td className="py-2.5 px-3 text-center text-xs">{t.charges.some(c => c.source === 'interest') ? '✓' : '—'}</td><td className="py-2.5 px-3 text-center text-xs">{t.charges.some(c => c.source === 'late_fee') ? '✓' : '—'}</td><td className="py-2.5 px-3 text-center text-xs text-zinc-500">{t.documents.length || '—'}</td><td className="py-2.5 px-3 text-center">{t.ready ? <span className="text-emerald-400 text-xs">✓</span> : <span className="text-amber-400 text-xs">⚠</span>}</td><td className="py-2.5 px-4 text-right text-white font-medium tabular-nums text-xs">R{t.total.toLocaleString()}</td></tr>))}</tbody>
                  </table>
                )}
              </div>
            </div>
