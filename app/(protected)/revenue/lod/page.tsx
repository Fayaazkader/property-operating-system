...
import { printLOD } from '@/lib/revenue/lod-print';
...
export default function LetterOfDemandPage() {
  ...
  useEffect(() => {
    async function init() {
      ...
      setTemplates(tpl || []);
      const [results, hist] = await Promise.all([lodEligibility.findEligible(entities[0]), lodQueue.getHistory(entities[0])]);
      setEligible(results); setHistory(hist || []);
    }
    init();
  }, []);
  ...
  async function handleSend() {
    if (!preview) return;
    await lodQueue.queue({ entityId, tenantId: preview.tenantId, templateId: preview.templateId, subject: preview.subject, body: preview.body });
    setPreview(null);
    ...
    <div className="p-8 max-w-5xl mx-auto space-y-8">
        ...
        <div className="space-y-2">
          {filtered.map(e => (
            <div key={e.tenantId} className="rounded-xl border border-white/[0.06] bg-white/[0.01] p-4 flex items-center justify-between">
              <div><p className="text-sm text-white font-light">{e.tenantName}</p><p className="text-xs text-zinc-500">R{e.overdueAmount.toLocaleString()} · {e.daysOverdue} days · {e.propertyName}</p></div>
              <button onClick={() => handlePreview(e.tenantId, e.matchedTemplateId)} className="text-xs text-amber-400 hover:text-amber-300 border border-amber-500/30 rounded-lg px-3 py-1.5">Preview {e.matchedTemplateName || 'LOD'}</button>
            </div>
          ))}
        </div>
      )}
      ...
      {history.length > 0 && (
        <div className="rounded-xl border border-white/[0.06] overflow-hidden">
          <div className="px-5 py-3 border-b border-white/[0.06] bg-white/[0.02]"><p className="text-[11px] font-medium text-zinc-500 uppercase">History</p></div>
          <table className="w-full text-sm">
            <thead><tr className="border-b border-white/[0.06]"><th className="text-left py-2 px-4 text-[10px] text-zinc-500">Tenant</th><th className="text-left py-2 px-4 text-[10px] text-zinc-500">Template</th><th className="text-left py-2 px-4 text-[10px] text-zinc-500">Status</th><th className="text-left py-2 px-4 text-[10px] text-zinc-500">Sent Via</th><th className="text-left py-2 px-4 text-[10px] text-zinc-500">Date</th><th className="text-left py-2 px-4 text-[10px] text-zinc-500"></th></tr></thead>
            <tbody>{history.map(h => (<tr key={h.id} className="border-b border-white/[0.03]"><td className="py-2 px-4 text-white text-xs font-light">{h.tenants?.tenant_name || '—'}</td><td className="py-2 px-4 text-zinc-400 text-xs">{h.templates?.name || '—'}</td><td className="py-2 px-4"><span className={`text-[10px] px-2 py-0.5 rounded-full ${h.status === 'queued' ? 'bg-amber-500/10 text-amber-400' : h.status === 'emailed' ? 'bg-emerald-500/10 text-emerald-400' : 'bg-zinc-800 text-zinc-500'}`}>{h.status}</span></td><td className="py-2 px-4 text-zinc-400 text-xs">{h.sent_via || '—'}</td><td className="py-2 px-4 text-zinc-500 text-xs">{h.created_at?.split('T')[0]}</td><td className="py-2 px-4"><button onClick={() => handleRegenerate(h.id)} className="text-[10px] text-zinc-500 hover:text-white">↻ Regenerate</button></td></tr>))}</tbody></table>
        </div>
      )}
        ...
        <>
          ...
          <div className="fixed inset-4 z-50 overflow-y-auto flex items-start justify-center p-4">
            <div className="bg-white text-black rounded-2xl p-8 max-w-2xl w-full max-h-[85vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
              <div className="flex justify-between items-center mb-4"><p className="text-sm font-medium">{preview.subject}</p><button onClick={() => setPreview(null)} className="text-zinc-500 hover:text-black">✕</button></div>
              ...
              <div className="flex gap-3 mt-6 pt-4 border-t border-zinc-200">
                ...
                <button onClick={() => printLOD(preview.tenantId, preview.templateId, entityId)} className="rounded-lg border border-zinc-300 px-4 py-2 text-xs text-zinc-600 hover:bg-zinc-100">Print</button>
              </div>
            </div>
          </div>
        </>
