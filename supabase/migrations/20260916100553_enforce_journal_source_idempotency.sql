-- Enforce financial journal idempotency by source identity.
--
-- A posting source is uniquely identified by:
--   source_event + source_id
--
-- This prevents the same business transaction from creating
-- multiple journals/GL entries when a posting operation is retried.

CREATE UNIQUE INDEX IF NOT EXISTS journals_source_event_source_id_unique
ON public.journals (source_event, source_id)
WHERE source_id IS NOT NULL;


-- Replace the atomic posting function with an idempotent version.
--
-- If the source transaction has already produced a journal, return
-- the existing journal ID instead of creating another journal/GL entry.
--
-- Journal + journal lines + GL entries remain one PostgreSQL transaction.
