...
export function BillingCard({ scale = 1 }: { scale?: number }) {
  return (
    <ProblemCard className="animate-float" style={{ width: 240, transform: `rotate(2deg) scale(${scale})`, animationDelay: '0.3s' }}>
      <div className="flex items-center gap-2 mb-3"><Receipt className="w-3.5 h-3.5 text-blue-400/70" /><span className="text-[10px] text-zinc-400 font-light">Billing System</span></div>
      ...
      <div className="mt-3 flex items-center gap-1.5"><div className="w-1.5 h-1.5 rounded-full bg-amber-400/60" /><span className="text-[10px] text-amber-400/80 font-light">Pending</span></div>
    </ProblemCard>
  );
}
